...
${renderStyle === 'stock' ? `9s. STOCK FOOTAGE MODE — for EVERY scene also add "stock_query": a 2-4 word search term for a REAL stock-video library (Pexels/Pixabay). Stock libraries only have generic real-world footage — so translate the scene's subject into the closest FILMABLE, EXISTING visual: concrete nouns + location/activity, no fictional characters, no invented details, no camera language. Examples: script about the Baghdad Battery → "ancient clay pottery"; pyramids as power plants → "egypt pyramids giza"; a medieval feast → "rustic food candlelight". The query must clearly relate to THIS scene's script_segment topic.` : ''}
