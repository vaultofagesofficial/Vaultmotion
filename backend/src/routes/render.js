...
router.post('/', async (req, res) => {
  try {
    const { script, title, style, duration, workspace_id, subtitleSettings, voiceKey, render_style, mode, format, preview, hybrid_intensity, illustration_style, ai_image_style, color_theme } = req.body;

    if (!script || script.trim().length < 10) {
      return res.status(400).json({ error: 'Script is verplicht (min. 10 tekens)' });
    }

    const result = await startRenderJob({ script, title, style, duration, workspace_id, subtitleSettings, voiceKey, render_style, mode, format, preview: !!preview, hybrid_intensity, illustration_style, ai_image_style, color_theme });
